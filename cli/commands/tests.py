...
class RunTestsActor(TestSuiteActor):
    ...
    def run_command_on_guest(self, config, guest, command_directory, working_directory, command):
        if not command:
            return
        ...
        with tempfile.NamedTemporaryFile(dir=command_directory) as f:
            ...
            self.vagrant(config, 'ssh', [guest], ['/shared/commands/{}'.format(name)])
    ...
    def task_test_case_step(self, task, config, command_directory, step, default_guest):
        ...
        try:
            self.run_command_on_guest(config, guest, command_directory, working_directory, command)
        finally:
